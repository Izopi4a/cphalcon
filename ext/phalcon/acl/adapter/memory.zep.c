...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addRole) {
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &roleObject, "__construct", NULL, 71, role);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addInherit) {
	...
	if (zephir_array_isset(&_3, &roleInheritName)) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_5$$6), _6$$6)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "addinherit", &_7, 72, &roleName, &deepInheritName);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addResource) {
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &resourceObject, "__construct", NULL, 73, &resourceName);
		zephir_check_call_status();
	}
