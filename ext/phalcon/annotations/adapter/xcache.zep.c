...
PHP_METHOD(Phalcon_Annotations_Adapter_Xcache, read) {
	...
	ZEPHIR_CALL_FUNCTION(&serialized, "xcache_get", NULL, 79, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(&serialized) == IS_STRING) {
		ZEPHIR_CALL_FUNCTION(&data, "unserialize", NULL, 60, &serialized);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Annotations_Adapter_Xcache, write) {
	...
	ZEPHIR_CONCAT_SV(&_1, "_PHAN", &key);
	zephir_fast_strtolower(&_0, &_1);
	ZEPHIR_CALL_FUNCTION(&_2, "serialize", NULL, 59, data);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 80, &_0, &_2);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
