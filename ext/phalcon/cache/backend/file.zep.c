...
PHP_METHOD(Phalcon_Cache_Backend_File, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_cache_backend_file_ce, this_ptr, "__construct", &_5, 0, frontend, &options);
...
PHP_METHOD(Phalcon_Cache_Backend_File, get) {
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS) == 1) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "clearstatcache", NULL, 100, &__$true, &cacheFile);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, delete) {
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("unlink", NULL, 101, &cacheFile);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Phalcon_Cache_Backend_File, queryKeys) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_4, "__construct", NULL, 102, &cacheDir);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, flush) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 102, &cacheDir);
	zephir_check_call_status();
	...
	for (;_1->funcs->valid(_1 TSRMLS_CC) == SUCCESS && !EG(exception); _1->funcs->move_forward(_1 TSRMLS_CC)) {
		...
		if (likely(ZEPHIR_IS_TRUE(&_3$$4))) {
			...
			if (_4$$5) {
				ZEPHIR_CALL_FUNCTION(&_5$$6, "unlink", &_6, 101, &cacheFile);
				zephir_check_call_status();
