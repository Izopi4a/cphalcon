...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_cache_backend_xcache_ce, this_ptr, "__construct", &_1, 0, frontend, options);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, get) {
	...
	ZEPHIR_CALL_FUNCTION(&cachedContent, "xcache_get", NULL, 79, &prefixedKey);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, save) {
	...
	ZEPHIR_CALL_FUNCTION(&success, "xcache_set", &_1, 80, &lastKey, &preparedContent, &tt1);
	zephir_check_call_status();
	...
	if (zephir_is_true(&success)) {
		...
		if (!ZEPHIR_IS_STRING(&specialKey, "")) {
			ZEPHIR_CALL_FUNCTION(&keys, "xcache_get", NULL, 79, &specialKey);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", &_1, 80, &specialKey, &keys);
			zephir_check_call_status();
		}
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, delete) {
	...
	if (!ZEPHIR_IS_STRING(&specialKey, "")) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 80, &specialKey, &keys);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		ZEPHIR_RETURN_CALL_FUNCTION("xcache_isset", NULL, 105, &lastKey);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, increment) {
	...
	if ((zephir_function_exists_ex(SL("xcache_inc") TSRMLS_CC) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_inc", NULL, 106, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&origVal, "xcache_get", NULL, 79, &lastKey);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&newVal);
		ZVAL_LONG(&newVal, (zephir_get_numberval(&origVal) - value));
		ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 80, &lastKey, &newVal);
		zephir_check_call_status();
	}
	RETURN_CCTOR(newVal);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, decrement) {
	...
	if ((zephir_function_exists_ex(SL("xcache_dec") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_1$$6, value);
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_dec", NULL, 107, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&origVal, "xcache_get", NULL, 79, &lastKey);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&newVal);
		ZVAL_LONG(&newVal, (zephir_get_numberval(&origVal) - value));
		ZEPHIR_CALL_FUNCTION(&success, "xcache_set", NULL, 80, &lastKey, &newVal);
		zephir_check_call_status();
	}
	RETURN_CCTOR(newVal);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, flush) {
	...
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _3$$5, _4$$5, _2$$5)
		{
			...
			ZEPHIR_CALL_FUNCTION(NULL, "xcache_unset", &_5, 108, &key);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&_1$$5);
		ZEPHIR_INIT_NVAR(&key);
