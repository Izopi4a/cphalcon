...
PHP_METHOD(Phalcon_Cli_Console, setArgument) {
	...
	if (unlikely(Z_TYPE_P(str_param) != IS_TRUE && Z_TYPE_P(str_param) != IS_FALSE)) {
	...
	if (unlikely(Z_TYPE_P(shift_param) != IS_TRUE && Z_TYPE_P(shift_param) != IS_FALSE)) {
	...
	if (_0) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "array_shift", &_1, 118, &arguments);
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arguments), _2)
	{
		...
		if (Z_TYPE_P(&arg) == IS_STRING) {
			...
			ZVAL_STRING(&_3$$5, "--");
			ZVAL_LONG(&_4$$5, 2);
			ZEPHIR_CALL_FUNCTION(&_5$$5, "strncmp", &_6, 119, &arg, &_3$$5, &_4$$5);
			zephir_check_call_status();
			if (ZEPHIR_IS_LONG(&_5$$5, 0)) {
				...
				ZVAL_STRING(&_18$$9, "-");
				ZVAL_LONG(&_19$$9, 1);
				ZEPHIR_CALL_FUNCTION(&_20$$9, "strncmp", &_6, 119, &arg, &_18$$9, &_19$$9);
				zephir_check_call_status();
	...
	if (str) {
		...
		if (zephir_fast_count_int(&args TSRMLS_CC)) {
			...
			ZEPHIR_CALL_FUNCTION(&_26$$15, "array_shift", &_1, 118, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
		...
		if (zephir_fast_count_int(&args TSRMLS_CC)) {
			ZEPHIR_MAKE_REF(&args);
			ZEPHIR_CALL_FUNCTION(&_27$$16, "array_shift", &_1, 118, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
