...
PHP_METHOD(Phalcon_Config, __construct) {
	...
	zephir_is_iterable(&arrayConfig, 0, "phalcon/config.zep", 63);
...
PHP_METHOD(Phalcon_Config, offsetSet) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 14, value);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config, merge) {
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "_merge", NULL, 15, config);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Config, toArray) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/config.zep", 196);
...
PHP_METHOD(Phalcon_Config, __set_state) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 14, &data);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Config, _merge) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_0, "get_object_vars", NULL, 16, config);
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/config.zep", 261);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _2, _3, _1)
	{
		ZEPHIR_INIT_NVAR(&key);
		if (_3 != NULL) { 
		...
		if (zephir_fetch_property_zval(&localObject, instance, &key, PH_SILENT_CC)) {
			...
			if (_4$$5) {
				...
				if (_5$$6) {
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "_merge", &_6, 15, &value, &localObject);
					zephir_check_call_status();
					continue;
				}
			}
		...
		if (zephir_is_numeric(&key)) {
			ZEPHIR_CALL_FUNCTION(&key, "strval", &_7, 13, &number);
			zephir_check_call_status();
