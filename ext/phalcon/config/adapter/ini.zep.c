...
PHP_METHOD(Phalcon_Config_Adapter_Ini, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&iniConfig, "parse_ini_file", NULL, 121, &filePath, &__$true, mode);
	...
	ZEPHIR_INIT_NVAR(&directives);
	ZEPHIR_INIT_NVAR(&section);
	ZEPHIR_CALL_PARENT(NULL, phalcon_config_adapter_ini_ce, this_ptr, "__construct", &_16, 0, &config);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Config_Adapter_Ini, _parseIniString) {
	...
	zephir_create_array(return_value, 1, 0 TSRMLS_CC);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "_parseinistring", NULL, 122, &path, value);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Ini, _cast) {
	...
	if (Z_TYPE_P(ini) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(ini), _1$$3, _2$$3, _0$$3)
		{
			...
			ZVAL_COPY(&val, _0$$3);
			ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "_cast", &_4, 123, &val);
			zephir_check_call_status();
