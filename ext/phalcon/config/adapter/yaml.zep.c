...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct) {
	...
	if (!ZEPHIR_IS_STRING_IDENTICAL(&callbacks, "")) {
		...
		ZEPHIR_CALL_FUNCTION(&yamlConfig, "yaml_parse_file", &_4, 125, &filePath, &_2$$4, &_3$$4, &callbacks);
		ZEPHIR_UNREF(&_3$$4);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&yamlConfig, "yaml_parse_file", &_4, 125, &filePath);
		zephir_check_call_status();
	}
	if (ZEPHIR_IS_FALSE_IDENTICAL(&yamlConfig)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_5$$6, "__construct", NULL, 4, &_7$$6);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_5$$6, "phalcon/config/adapter/yaml.zep", 86 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_PARENT(NULL, phalcon_config_adapter_yaml_ce, this_ptr, "__construct", &_8, 0, &yamlConfig);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
