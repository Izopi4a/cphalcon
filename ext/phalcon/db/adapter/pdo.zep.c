...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_db_adapter_pdo_ce, this_ptr, "__construct", &_0, 0, &descriptor);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, executePrepared) {
	...
	zephir_is_iterable(&placeholders, 0, "phalcon/db/adapter/pdo.zep", 306);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&placeholders), _1, _2, _0)
	{
		...
		if (_3$$3) {
					...
					if (Z_TYPE_P(&value) != IS_ARRAY) {
						do {
							if (ZEPHIR_IS_LONG(&type, 1)) {
								ZVAL_LONG(&_4$$12, 10);
								ZEPHIR_CALL_FUNCTION(&castValue, "intval", &_5, 30, &value, &_4$$12);
								zephir_check_call_status();
								break;
							}
			...
			if (Z_TYPE_P(&castValue) != IS_ARRAY) {
				...
				zephir_is_iterable(&castValue, 0, "phalcon/db/adapter/pdo.zep", 294);
			...
			if (Z_TYPE_P(&value) != IS_ARRAY) {
				...
				zephir_is_iterable(&value, 0, "phalcon/db/adapter/pdo.zep", 302);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, query) {
	...
	if (Z_TYPE_P(&statement) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 31, this_ptr, &statement, &sqlStatement, bindParams, bindTypes);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, convertBoundParams) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "preg_match_all", NULL, 32, &bindPattern, &sql, &matches, &_0);
	ZEPHIR_UNREF(&matches);
	zephir_check_call_status();
	if (zephir_is_true(&_1)) {
		zephir_is_iterable(&matches, 0, "phalcon/db/adapter/pdo.zep", 515);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&matches), _2$$3)
		{
			ZEPHIR_INIT_NVAR(&placeMatch);
			ZVAL_COPY(&placeMatch, _2$$3);
			ZEPHIR_OBS_NVAR(&value);
			zephir_array_fetch_long(&_3$$4, &placeMatch, 1, PH_READONLY, "phalcon/db/adapter/pdo.zep", 502 TSRMLS_CC);
			if (!(zephir_array_isset_fetch(&value, &params, &_3$$4, 0 TSRMLS_CC))) {
				if (zephir_array_isset_long(&placeMatch, 2)) {
					ZEPHIR_OBS_NVAR(&value);
					zephir_array_fetch_long(&_4$$6, &placeMatch, 2, PH_READONLY, "phalcon/db/adapter/pdo.zep", 504 TSRMLS_CC);
		...
		ZVAL_STRING(&_5$$3, "?");
		ZEPHIR_CALL_FUNCTION(&boundSql, "preg_replace", NULL, 33, &bindPattern, &_5$$3, &sql);
		zephir_check_call_status();
	} else {
