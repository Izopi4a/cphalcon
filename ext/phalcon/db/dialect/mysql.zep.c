		...
		if (!(ZEPHIR_IS_EMPTY(&typeValues))) {
			if (Z_TYPE_P(&typeValues) == IS_ARRAY) {
				...
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&typeValues), _23$$51)
				{
					...
					ZVAL_STRING(&_24$$52, "\"");
					ZEPHIR_CALL_FUNCTION(&_25$$52, "addcslashes", &_26, 137, &value$$51, &_24$$52);
					zephir_check_call_status();
				...
				ZVAL_STRING(&_32$$53, "\"");
				ZEPHIR_CALL_FUNCTION(&_33$$53, "addcslashes", &_26, 137, &typeValues, &_32$$53);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect_Mysql, addColumn) {
	...
	if (zephir_is_true(&_3)) {
		...
		if (zephir_memnstr_str(&_4$$3, SL("CURRENT_TIMESTAMP"), "phalcon/db/dialect/mysql.zep", 229)) {
			...
			ZVAL_STRING(&_5$$5, "\"");
			ZEPHIR_CALL_FUNCTION(&_6$$5, "addcslashes", NULL, 137, &defaultValue, &_5$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect_Mysql, addIndex) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "getcolumnlist", NULL, 48, &_4);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_5);
...
PHP_METHOD(Phalcon_Db_Dialect_Mysql, addPrimaryKey) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "getcolumnlist", NULL, 48, &_2);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect_Mysql, addForeignKey) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *tableName_param = NULL, *schemaName_param = NULL, *reference, reference_sub, sql, onDelete, onUpdate, _0, _1, _2, _3, _4, _5, _6, _7, _8, _9$$3, _10$$4;
	zval tableName, schemaName;
	ZEPHIR_INIT_THIS();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "getcolumnlist", NULL, 48, &_3);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_7, this_ptr, "getcolumnlist", NULL, 48, &_8);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&sql);
	...
	if (!(ZEPHIR_IS_EMPTY(&onDelete))) {
		...
		ZEPHIR_CONCAT_SV(&_9$$3, " ON DELETE ", &onDelete);
	...
	if (!(ZEPHIR_IS_EMPTY(&onUpdate))) {
		...
		ZEPHIR_CONCAT_SV(&_10$$4, " ON UPDATE ", &onUpdate);
...
PHP_METHOD(Phalcon_Db_Dialect_Mysql, createTable) {
	...
	if (!(zephir_array_isset_string_fetch(&columns, &definition, SL("columns"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The index 'columns' is required in the definition array", "phalcon/db/dialect/mysql.zep", 385);
		return;
	}
	...
	zephir_is_iterable(&columns, 0, "phalcon/db/dialect/mysql.zep", 448);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&columns), _0)
	{
		...
		if (zephir_is_true(&_4$$7)) {
			...
			if (zephir_memnstr_str(&_5$$8, SL("CURRENT_TIMESTAMP"), "phalcon/db/dialect/mysql.zep", 414)) {
				zephir_concat_self_str(&columnLine, SL(" DEFAULT CURRENT_TIMESTAMP") TSRMLS_CC);
			} else {
				ZEPHIR_INIT_NVAR(&_6$$10);
				ZVAL_STRING(&_6$$10, "\"");
				ZEPHIR_CALL_FUNCTION(&_7$$10, "addcslashes", &_8, 137, &defaultValue, &_6$$10);
				zephir_check_call_status();
		...
		zephir_array_append(&createLines, &columnLine, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 442);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&column);
	ZEPHIR_OBS_VAR(&indexes);
	if (zephir_array_isset_string_fetch(&indexes, &definition, SL("indexes"), 0)) {
		zephir_is_iterable(&indexes, 0, "phalcon/db/dialect/mysql.zep", 470);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&indexes), _13$$14)
		{
			...
			if (ZEPHIR_IS_STRING(&indexName, "PRIMARY")) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_14$$16, this_ptr, "getcolumnlist", &_16, 48, &_15$$16);
				zephir_check_call_status();
				...
				if (!(ZEPHIR_IS_EMPTY(&indexType))) {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_17$$18, this_ptr, "getcolumnlist", &_16, 48, &_18$$18);
					zephir_check_call_status();
					ZEPHIR_CONCAT_VSVSVS(&indexSql, &indexType, " KEY `", &indexName, "` (", &_17$$18, ")");
				} else {
					ZEPHIR_CALL_METHOD(&_20$$19, &index, "getcolumns", NULL, 0);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_19$$19, this_ptr, "getcolumnlist", &_16, 48, &_20$$19);
					zephir_check_call_status();
					ZEPHIR_CONCAT_SVSVS(&indexSql, "KEY `", &indexName, "` (", &_19$$19, ")");
				}
			}
			zephir_array_append(&createLines, &indexSql, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 468);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&index);
	}
	ZEPHIR_OBS_VAR(&references);
	if (zephir_array_isset_string_fetch(&references, &definition, SL("references"), 0)) {
		zephir_is_iterable(&references, 0, "phalcon/db/dialect/mysql.zep", 492);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&references), _21$$20)
		{
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_23$$21, this_ptr, "getcolumnlist", &_16, 48, &_24$$21);
			zephir_check_call_status();
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_26$$21, this_ptr, "getcolumnlist", &_16, 48, &_27$$21);
			zephir_check_call_status();
			...
			zephir_array_append(&createLines, &referenceSql, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 490);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&reference);
	}
	ZEPHIR_INIT_VAR(&_30);
...
PHP_METHOD(Phalcon_Db_Dialect_Mysql, createView) {
	...
	if (!(zephir_array_isset_string_fetch(&viewSql, &definition, SL("sql"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The index 'sql' is required in the definition array", "phalcon/db/dialect/mysql.zep", 546);
		return;
	}
	...
	if (zephir_array_isset_string_fetch(&options, &definition, SL("options"), 0)) {
		...
		if (zephir_array_isset_string_fetch(&engine, &options, SL("ENGINE"), 0)) {
			if (zephir_is_true(&engine)) {
				...
				zephir_array_append(&tableOptions, &_0$$5, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 691);
			}
		}
		...
		if (zephir_array_isset_string_fetch(&autoIncrement, &options, SL("AUTO_INCREMENT"), 0)) {
			if (zephir_is_true(&autoIncrement)) {
				...
				zephir_array_append(&tableOptions, &_1$$7, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 700);
			}
		}
		...
		if (zephir_array_isset_string_fetch(&tableCollation, &options, SL("TABLE_COLLATION"), 0)) {
			if (zephir_is_true(&tableCollation)) {
				...
				zephir_array_fetch_long(&_2$$9, &collationParts, 0, PH_NOISY | PH_READONLY, "phalcon/db/dialect/mysql.zep", 710 TSRMLS_CC);
				ZEPHIR_INIT_VAR(&_3$$9);
				ZEPHIR_CONCAT_SV(&_3$$9, "DEFAULT CHARSET=", &_2$$9);
				zephir_array_append(&tableOptions, &_3$$9, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 710);
				ZEPHIR_INIT_VAR(&_4$$9);
				ZEPHIR_CONCAT_SV(&_4$$9, "COLLATE=", &tableCollation);
				zephir_array_append(&tableOptions, &_4$$9, PH_SEPARATE, "phalcon/db/dialect/mysql.zep", 711);
			}
		}
