...
PHP_METHOD(Phalcon_Di_Service_Builder, _buildParameters) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&arguments), _1, _2, _0)
	{
		...
		ZEPHIR_CALL_METHOD(&_3$$3, this_ptr, "_buildparameter", &_4, 165, dependencyInjector, &position, &argument);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Service_Builder, build) {
	...
	if (Z_TYPE_P(parameters) == IS_ARRAY) {
		...
		if (zephir_array_isset_string_fetch(&arguments, &definition, SL("arguments"), 0)) {
			ZEPHIR_CALL_METHOD(&_0$$8, this_ptr, "_buildparameters", &_1, 166, dependencyInjector, &arguments);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&instance);
	...
	if (zephir_array_isset_string_fetch(&paramCalls, &definition, SL("calls"), 0)) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&paramCalls), _3$$10, _4$$10, _2$$10)
		{
			...
			if (Z_TYPE_P(&method) != IS_ARRAY) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_5$$14, "phalcon/di/service/builder.zep", 195 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (!(zephir_array_isset_string_fetch(&methodName, &method, SL("method"), 0))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_8$$15, "phalcon/di/service/builder.zep", 202 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (zephir_array_isset_string_fetch(&arguments, &method, SL("arguments"), 0)) {
				if (Z_TYPE_P(&arguments) != IS_ARRAY) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_10$$17, "phalcon/di/service/builder.zep", 213 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
				if (zephir_fast_count_int(&arguments TSRMLS_CC)) {
					ZEPHIR_INIT_NVAR(&_12$$18);
					ZEPHIR_CALL_METHOD(&_13$$18, this_ptr, "_buildparameters", &_1, 166, dependencyInjector, &arguments);
					zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&paramCalls, &definition, SL("properties"), 0)) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&paramCalls), _16$$19, _17$$19, _15$$19)
		{
			ZEPHIR_INIT_NVAR(&propertyPosition);
			if (_17$$19 != NULL) { 
				ZVAL_STR_COPY(&propertyPosition, _17$$19);
			} else {
				ZVAL_LONG(&propertyPosition, _16$$19);
			}
			ZEPHIR_INIT_NVAR(&property);
			ZVAL_COPY(&property, _15$$19);
			if (Z_TYPE_P(&property) != IS_ARRAY) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_18$$23, "__construct", &_7, 4, &_19$$23);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_18$$23, "phalcon/di/service/builder.zep", 261 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (!(zephir_array_isset_string_fetch(&propertyName, &property, SL("name"), 0))) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_20$$24, "__construct", &_7, 4, &_21$$24);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_20$$24, "phalcon/di/service/builder.zep", 268 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (!(zephir_array_isset_string_fetch(&propertyValue, &property, SL("value"), 0))) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_22$$25, "__construct", &_7, 4, &_23$$25);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_22$$25, "phalcon/di/service/builder.zep", 275 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			ZEPHIR_CALL_METHOD(&_24$$22, this_ptr, "_buildparameter", &_25, 165, dependencyInjector, &propertyPosition, &propertyValue);
			zephir_check_call_status();
