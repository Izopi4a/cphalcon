...
PHP_METHOD(Phalcon_Dispatcher, setModelBinding) {
	...
	if (value) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 63, cache);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Dispatcher, _dispatch) {
	...
	while (1) {
		...
		if (zephir_is_true(&_49$$6)) {
			...
			ZEPHIR_CALL_METHOD(&_50$$39, &modelBinder, "bindtohandler", NULL, 0, &handler, &params, &bindCacheKey, &actionMethod);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(&params, &_50$$39);
		}
		zephir_update_property_zval(this_ptr, SL("_lastHandler"), &handler);
		if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
			ZEPHIR_INIT_NVAR(&_52$$40);
			ZVAL_STRING(&_52$$40, "dispatch:afterBinding");
			ZEPHIR_CALL_METHOD(&_51$$40, &eventsManager, "fire", &_53, 0, &_52$$40, this_ptr);
			zephir_check_call_status();
			if (ZEPHIR_IS_FALSE_IDENTICAL(&_51$$40)) {
				continue;
			}
			zephir_read_property(&_54$$40, this_ptr, SL("_finished"), PH_NOISY_CC | PH_READONLY);
			if (ZEPHIR_IS_FALSE_IDENTICAL(&_54$$40)) {
				continue;
			}
		}
		if ((zephir_method_exists_ex(&handler, SL("afterbinding") TSRMLS_CC) == SUCCESS)) {
			ZEPHIR_CALL_METHOD(&_55$$43, &handler, "afterbinding", NULL, 0, this_ptr);
			zephir_check_call_status();
			if (ZEPHIR_IS_FALSE_IDENTICAL(&_55$$43)) {
				continue;
			}
			zephir_read_property(&_56$$43, this_ptr, SL("_finished"), PH_NOISY_CC | PH_READONLY);
		...
		/* try_start_1: */
			...
			ZEPHIR_CALL_METHOD(&_57$$46, this_ptr, "callactionmethod", &_58, 0, &handler, &actionMethod, &params);
			zephir_check_call_status_or_jump(try_end_1);
			zephir_update_property_zval(this_ptr, SL("_returnedValue"), &_57$$46);
		...
		if (EG(exception)) {
			...
			if (zephir_instance_of_ev(&e, zend_exception_get_default(TSRMLS_C) TSRMLS_CC)) {
				...
				if (ZEPHIR_IS_FALSE_IDENTICAL(&_59$$47)) {
					zephir_read_property(&_61$$48, this_ptr, SL("_finished"), PH_NOISY_CC | PH_READONLY);
		...
		if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
			...
			ZVAL_STRING(&_63$$51, "dispatch:afterExecuteRoute");
			ZEPHIR_CALL_METHOD(&_62$$51, &eventsManager, "fire", &_64, 0, &_63$$51, this_ptr, &value);
			zephir_check_call_status();
			if (ZEPHIR_IS_FALSE_IDENTICAL(&_62$$51)) {
				continue;
			}
			zephir_read_property(&_65$$51, this_ptr, SL("_finished"), PH_NOISY_CC | PH_READONLY);
			if (ZEPHIR_IS_FALSE_IDENTICAL(&_65$$51)) {
				continue;
			}
			ZEPHIR_INIT_NVAR(&_63$$51);
			ZVAL_STRING(&_63$$51, "dispatch:afterDispatch");
		...
		if ((zephir_method_exists_ex(&handler, SL("afterexecuteroute") TSRMLS_CC) == SUCCESS)) {
			...
			zephir_read_property(&_68$$54, this_ptr, SL("_finished"), PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
		...
		ZVAL_STRING(&_69$$57, "dispatch:afterDispatchLoop");
...
PHP_METHOD(Phalcon_Dispatcher, getHandlerClass) {
	...
	zephir_read_property(&namespaceName, this_ptr, SL("_namespaceName"), PH_NOISY_CC);
	if (!(zephir_memnstr_str(&handlerName, SL("\\"), "phalcon/dispatcher.zep", 780))) {
