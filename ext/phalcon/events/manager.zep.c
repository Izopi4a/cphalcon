...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC))) {
		...
		if (zephir_is_true(&_1$$4)) {
			...
			ZVAL_LONG(&_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "setextractflags", NULL, 173, &_2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "insert", NULL, 174, handler, &_3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(&_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "setextractflags", NULL, 173, &_1$$5);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$5, 3);
			...
			while (1) {
				...
				if (!ZEPHIR_IS_IDENTICAL(&_5$$6, handler)) {
					...
					ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "insert", &_8, 174, &_6$$7, &_7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), &eventType, &newPriorityQueue TSRMLS_CC);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 175, handler, &priorityQueue, &__$true);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fireQueue) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *queue, queue_sub, *event, event_sub, status, arguments, eventName, data, iterator, source, handler, _4, _5, _0$$5, _1$$5, _2$$5, _3$$5, _6$$8, _9$$14, _10$$19, *_11$$21, _12$$27, _13$$32;
	ZEPHIR_INIT_THIS();
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue TSRMLS_CC))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 176, &_2$$5, &_1$$5);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(queue) == IS_OBJECT) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(queue), _11$$21)
		{
			ZEPHIR_INIT_NVAR(&handler);
			ZVAL_COPY(&handler, _11$$21);
			if (Z_TYPE_P(&handler) == IS_OBJECT) {
				if (zephir_instance_of_ev(&handler, zend_ce_closure TSRMLS_CC)) {
					if (Z_TYPE_P(&arguments) == IS_NULL) {
						ZEPHIR_INIT_NVAR(&arguments);
					...
					if (cancelable) {
						ZEPHIR_CALL_METHOD(&_12$$27, event, "isstopped", NULL, 0);
						zephir_check_call_status();
						if (zephir_is_true(&_12$$27)) {
							break;
						}
					}
				} else {
					if ((zephir_method_exists(&handler, &eventName TSRMLS_CC)  == SUCCESS)) {
						...
						if (cancelable) {
							ZEPHIR_CALL_METHOD(&_13$$32, event, "isstopped", NULL, 0);
							zephir_check_call_status();
							if (zephir_is_true(&_13$$32)) {
								break;
							}
						}
					}
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	zval *eventType_param = NULL, *source, source_sub, *data = NULL, data_sub, *cancelable_param = NULL, __$null, events, eventParts, type, eventName, event, status, fireEvents, _2, _0$$4, _4$$7, _8$$10;
	zval eventType, _1$$4;
	ZEPHIR_INIT_THIS();
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0 TSRMLS_CC)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, &event, "__construct", &_5, 177, &eventName, source, data, &_4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_6, 178, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &eventType, 0 TSRMLS_CC)) {
		...
		if (_7$$8) {
			if (Z_TYPE_P(&event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, &event, "__construct", &_5, 177, &eventName, source, data, &_8$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_6, 178, &fireEvents, &event);
			zephir_check_call_status();
