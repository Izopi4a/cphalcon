...
PHP_METHOD(Phalcon_Filter, add) {
	...
	if (_0) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_filter_exception_ce, "Filter must be an object or callable", "phalcon/filter.zep", 75);
		return;
	}
...
PHP_METHOD(Phalcon_Filter, sanitize) {
	...
	if (Z_TYPE_P(filters) == IS_ARRAY) {
		if (Z_TYPE_P(value) != IS_NULL) {
			zephir_is_iterable(filters, 0, "phalcon/filter.zep", 108);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(filters), _0$$4)
			{
				...
				if (_1$$5) {
					...
					zephir_is_iterable(value, 0, "phalcon/filter.zep", 103);
	...
	if (_8) {
		...
		zephir_is_iterable(value, 0, "phalcon/filter.zep", 120);
...
PHP_METHOD(Phalcon_Filter, _sanitize) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval filter, _15$$19;
	zval *value, value_sub, *filter_param = NULL, filterObject, _0, _4$$5, _5$$5, _7$$6, _8$$8, _9$$9, _11$$10, _12$$12, _13$$12, _14$$19;
	ZEPHIR_INIT_THIS();
	...
	do {
		if (ZEPHIR_IS_STRING(&filter, "email")) {
			...
			zephir_check_call_status();
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 180, value, &_5$$5);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "int")) {
			ZVAL_LONG(&_7$$6, 519);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 180, value, &_7$$6);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "int!")) {
		...
		if (ZEPHIR_IS_STRING(&filter, "absint")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("abs", NULL, 181, &_8$$8);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "string")) {
			ZVAL_LONG(&_9$$9, 513);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 180, value, &_9$$9);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "float")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 180, value, &_11$$10, &_10$$10);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "float!")) {
		...
		if (ZEPHIR_IS_STRING(&filter, "alphanum")) {
			...
			ZVAL_STRING(&_13$$12, "");
			ZEPHIR_RETURN_CALL_FUNCTION("preg_replace", NULL, 33, &_12$$12, &_13$$12, value);
			zephir_check_call_status();
			RETURN_MM();
		}
		...
		if (ZEPHIR_IS_STRING(&filter, "striptags")) {
			ZEPHIR_RETURN_CALL_FUNCTION("strip_tags", NULL, 182, value);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "lower")) {
			if ((zephir_function_exists_ex(SL("mb_strtolower") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_RETURN_CALL_FUNCTION("mb_strtolower", NULL, 183, value);
				zephir_check_call_status();
				RETURN_MM();
			}
		...
		object_init_ex(&_14$$19, phalcon_filter_exception_ce);
		ZEPHIR_INIT_VAR(&_15$$19);
		ZEPHIR_CONCAT_SVS(&_15$$19, "Sanitize filter '", &filter, "' is not supported");
		ZEPHIR_CALL_METHOD(NULL, &_14$$19, "__construct", NULL, 4, &_15$$19);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_14$$19, "phalcon/filter.zep", 214 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	} while(0);
