...
PHP_METHOD(Phalcon_Http_Cookie, send) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 191, &name, &encryptValue, &expire, &path, &domain, &secure, &httpOnly);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_Cookie, delete) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 191, &name, &__$null, &_0, &path, &domain, &secure, &httpOnly);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
