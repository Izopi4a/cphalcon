...
PHP_METHOD(Phalcon_Http_Request_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "defined", NULL, 207, &_0$$3);
		zephir_check_call_status();
		if (zephir_is_true(&_1$$3)) {
			ZVAL_LONG(&_2$$4, 4);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "pathinfo", NULL, 67, &name, &_2$$4);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType) {
	...
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 209, &finfo, &_0);
	zephir_check_call_status();
