...
PHP_METHOD(Phalcon_Mvc_Model_Binder, bindToHandler) {
	...
	zval cacheKey;
	zval params;
	zval *handler, handler_sub, *params_param = NULL, *cacheKey_param = NULL, *methodName = NULL, methodName_sub, __$null, paramKey, className, boundModel, paramsCache, paramValue, _0, _2$$3, *_3$$4;
	ZEPHIR_INIT_THIS();
	...
	if (_1) {
		...
		if (Z_TYPE_P(&paramsCache) == IS_ARRAY) {
			zephir_is_iterable(&paramsCache, 0, "phalcon/mvc/model/binder.zep", 102);
...
PHP_METHOD(Phalcon_Mvc_Model_Binder, getParamsFromReflection) {
	...
	zval cacheKey;
	zval params;
	zval *handler, handler_sub, *params_param = NULL, *cacheKey_param = NULL, *methodName, methodName_sub, methodParams, reflection, paramKey, methodParam, paramsCache, className, realClasses, boundModel, cache, handlerClass, reflectionClass, paramsKeys, paramValue, _0, *_1, _4$$7, _5$$5, _6$$5, _10$$15, _11$$15;
	ZEPHIR_INIT_THIS();
	...
	if (Z_TYPE_P(methodName) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 259, handler, methodName);
		zephir_check_call_status();
	} else {
		object_init_ex(&reflection, zephir_get_internal_ce(SL("reflectionfunction")));
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 74, handler);
		zephir_check_call_status();
	}
	zephir_read_property(&_0, this_ptr, SL("cache"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&cache, &_0);
	ZEPHIR_CALL_METHOD(&methodParams, &reflection, "getparameters", NULL, 75);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&paramsKeys);
	zephir_array_keys(&paramsKeys, &params TSRMLS_CC);
	zephir_is_iterable(&methodParams, 0, "phalcon/mvc/model/binder.zep", 197);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&methodParams), _2, _3, _1)
	{
		...
		ZEPHIR_CALL_FUNCTION(&_6$$5, "is_subclass_of", &_7, 260, &className, &_5$$5);
		zephir_check_call_status();
		if (ZEPHIR_IS_STRING(&className, "Phalcon\\Mvc\\Model")) {
			...
			if (Z_TYPE_P(&realClasses) == IS_ARRAY) {
				...
				if (zephir_array_isset_fetch(&className, &realClasses, &paramKey, 0 TSRMLS_CC)) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_10$$15, "__construct", &_12, 4, &_11$$15);
					zephir_check_call_status();
