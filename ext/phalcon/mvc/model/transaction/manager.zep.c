...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, get) {
	...
	if (!(zephir_is_true(&_0))) {
		...
		if (zephir_is_true(&_1$$3)) {
			...
			ZEPHIR_CALL_FUNCTION(NULL, "register_shutdown_function", NULL, 307, &_2$$4);
			zephir_check_call_status();
		}
		if (1) {
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, getOrCreateTransaction) {
	...
	ZEPHIR_CALL_METHOD(NULL, &transaction, "__construct", NULL, 308, &dependencyInjector, &_4, &_3);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &transaction, "settransactionmanager", NULL, 309, this_ptr);
	zephir_check_call_status();
