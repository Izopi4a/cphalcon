...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, handle) {
		...
		if (Z_TYPE_P(&methodAnnotations) == IS_ARRAY) {
			...
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&methodAnnotations), _9$$16, _10$$16, _8$$16)
			{
				...
				if (Z_TYPE_P(&collection) == IS_OBJECT) {
					...
					zephir_check_call_status();
					zephir_is_iterable(&_11$$18, 0, "phalcon/mvc/router/annotations.zep", 197);
	...
	ZEPHIR_INIT_NVAR(&scope);
	ZEPHIR_CALL_PARENT(NULL, phalcon_mvc_router_annotations_ce, this_ptr, "handle", &_14, 0, &realUri);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, processActionAnnotation) {
	...
	if (isRoute == 1) {
		...
		if (Z_TYPE_P(&converts) == IS_ARRAY) {
			zephir_is_iterable(&converts, 0, "phalcon/mvc/router/annotations.zep", 339);
		...
		zephir_check_call_status();
		if (Z_TYPE_P(&converts) == IS_ARRAY) {
			zephir_is_iterable(&converts, 0, "phalcon/mvc/router/annotations.zep", 349);
