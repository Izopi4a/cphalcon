...
PHP_METHOD(Phalcon_Validation_Validator_Alpha, validate) {
	...
	zval field;
	zval *validation, validation_sub, *field_param = NULL, value, message, label, replacePairs, code, _0, _1, _2, _3$$3, _8$$3, _9$$3, _4$$4, _5$$6, _6$$7, _7$$8;
	ZEPHIR_INIT_THIS();
	...
	if (zephir_is_true(&_1)) {
		...
		if (Z_TYPE_P(&label) == IS_ARRAY) {
			zephir_array_fetch(&_4$$4, &label, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/alpha.zep", 74 TSRMLS_CC);
		...
		if (Z_TYPE_P(&message) == IS_ARRAY) {
			zephir_array_fetch(&_5$$6, &message, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/alpha.zep", 82 TSRMLS_CC);
		...
		if (ZEPHIR_IS_EMPTY(&message)) {
			...
			ZVAL_STRING(&_6$$7, "Alpha");
			ZEPHIR_CALL_METHOD(&message, validation, "getdefaultmessage", NULL, 0, &_6$$7);
			zephir_check_call_status();
		}
		ZEPHIR_INIT_NVAR(&_3$$3);
		ZVAL_STRING(&_3$$3, "code");
		ZEPHIR_CALL_METHOD(&code, this_ptr, "getoption", NULL, 0, &_3$$3);
		zephir_check_call_status();
		if (Z_TYPE_P(&code) == IS_ARRAY) {
			zephir_array_fetch(&_7$$8, &code, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/alpha.zep", 91 TSRMLS_CC);
		...
		ZVAL_STRING(&_9$$3, "Alpha");
		ZEPHIR_CALL_METHOD(NULL, &_3$$3, "__construct", NULL, 385, &_8$$3, &field, &_9$$3, &code);
		zephir_check_call_status();
