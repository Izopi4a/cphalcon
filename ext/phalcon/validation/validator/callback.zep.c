...
PHP_METHOD(Phalcon_Validation_Validator_Callback, validate) {
	...
	zval field;
	zval *validation, validation_sub, *field_param = NULL, message, label, replacePairs, code, callback, returnedValue, data, _0, _2$$6, _7$$6, _8$$6, _3$$7, _4$$9, _5$$10, _6$$11;
	ZEPHIR_INIT_THIS();
	...
	if (zephir_is_callable(&callback TSRMLS_CC)) {
		...
		ZEPHIR_CALL_FUNCTION(&returnedValue, "call_user_func", NULL, 327, &callback, &data);
		zephir_check_call_status();
		...
		if (((Z_TYPE_P(&returnedValue) == IS_TRUE || Z_TYPE_P(&returnedValue) == IS_FALSE) == 1)) {
			if (!(zephir_is_true(&returnedValue))) {
				...
				ZVAL_STRING(&_8$$6, "Callback");
				ZEPHIR_CALL_METHOD(NULL, &_2$$6, "__construct", NULL, 385, &_7$$6, &field, &_8$$6, &code);
				zephir_check_call_status();
