...
PHP_METHOD(Phalcon_Validation_Validator_Confirmation, validate) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "compare", NULL, 388, &value, &valueWith);
	zephir_check_call_status();
	if (!(zephir_is_true(&_2))) {
		...
		if (Z_TYPE_P(&label) == IS_ARRAY) {
			zephir_array_fetch(&_4$$5, &label, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/confirmation.zep", 88 TSRMLS_CC);
		...
		if (Z_TYPE_P(&labelWith) == IS_ARRAY) {
			zephir_array_fetch(&_5$$7, &labelWith, &fieldWith, PH_NOISY | PH_READONLY, "phalcon/validation/validator/confirmation.zep", 96 TSRMLS_CC);
		...
		if (Z_TYPE_P(&message) == IS_ARRAY) {
			zephir_array_fetch(&_6$$9, &message, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/confirmation.zep", 104 TSRMLS_CC);
		...
		if (Z_TYPE_P(&code) == IS_ARRAY) {
			zephir_array_fetch(&_8$$11, &code, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/confirmation.zep", 114 TSRMLS_CC);
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$4, "__construct", NULL, 385, &_9$$4, &field, &_10$$4, &code);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_Confirmation, compare) {
	...
	if (zephir_is_true(&_0)) {
		if (!((zephir_function_exists_ex(SL("mb_strtolower") TSRMLS_CC) == SUCCESS))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Extension 'mbstring' is required", "phalcon/validation/validator/confirmation.zep", 135);
			return;
		}
		ZEPHIR_INIT_VAR(&_3$$3);
		ZVAL_STRING(&_3$$3, "utf-8");
		ZEPHIR_CALL_FUNCTION(&_4$$3, "mb_strtolower", &_5, 183, &a, &_3$$3);
		zephir_check_call_status();
		zephir_get_strval(&a, &_4$$3);
		ZEPHIR_INIT_NVAR(&_3$$3);
		ZVAL_STRING(&_3$$3, "utf-8");
		ZEPHIR_CALL_FUNCTION(&_6$$3, "mb_strtolower", &_5, 183, &b, &_3$$3);
		zephir_check_call_status();
