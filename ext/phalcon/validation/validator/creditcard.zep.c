...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	ZEPHIR_CALL_METHOD(&valid, this_ptr, "verifybyluhnalgorithm", NULL, 389, &value);
	zephir_check_call_status();
	if (!(zephir_is_true(&valid))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 385, &_5$$3, &field, &_6$$3, &code);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "str_split", &_1, 65, number);
	zephir_check_call_status();
	zephir_get_arrval(&_2, &_0);
	ZEPHIR_CPY_WRT(&digits, &_2);
	ZEPHIR_INIT_VAR(&hash);
	ZVAL_STRING(&hash, "");
	ZEPHIR_CALL_FUNCTION(&_4, "array_reverse", NULL, 369, &digits);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&digit);
	ZEPHIR_INIT_NVAR(&position);
	ZEPHIR_CALL_FUNCTION(&_9, "str_split", &_1, 65, &hash);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 390, &_9);
	zephir_check_call_status();
