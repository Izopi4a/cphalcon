...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, validate) {
	...
	if (!(zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$3, "strtr", NULL, 21, &message, &_3$$3);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 385, &_4$$3, field, &_6$$3, &_5$$3);
		zephir_check_call_status();
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _1)
	{
		...
		if (zephir_is_true(&except)) {
			...
			if (_8$$6) {
				...
				ZEPHIR_CALL_FUNCTION(&_12$$6, "range", &_13, 373, &_10$$6, &_11$$6);
				zephir_check_call_status();
